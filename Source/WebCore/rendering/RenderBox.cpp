...
FloatOrLayoutRect RenderBox::referenceBoxRect(CSSBoxType boxType) const
{
    switch (boxType) {
...
bool RenderBox::hitTestClipPath(const HitTestLocation& hitTestLocation, const LayoutPoint& accumulatedOffset) const
{
    ...
    case PathOperation::Shape: {
        ...
        if (!clipPath.pathForReferenceRect(referenceBoxRect(clipPath.referenceBox())).contains(hitTestLocationInLocalCoordinates, clipPath.windRule()))
            return false;
        break;
    }
