...
{
#if ENABLE(LAYER_BASED_SVG_ENGINE)
    bool usesSVGSubtreeTransformRules = renderer().document().settings().layerBasedSVGEngineEnabled() && renderer().isSVGLayerAwareRenderer() && !renderer().isSVGRoot();
#else
    bool usesSVGSubtreeTransformRules = false;
#endif
    ...
    FloatSize alignedOffsetForThisLayer = usesSVGSubtreeTransformRules ? offsetForThisLayer : toFloatSize(roundPointToDevicePixels(toLayoutPoint(offsetForThisLayer), deviceScaleFactor));
    ...
    if (!usesSVGSubtreeTransformRules && !renderer().isSVGRoot())
