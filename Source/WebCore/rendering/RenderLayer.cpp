...
FloatRect RenderLayer::referenceBoxRectForClipping(CSSBoxType boxType, const LayoutSize& offsetFromRoot, const LayoutRect& rootRelativeBounds) const
{
...
void RenderLayer::updateTransform()
{
    ...
    if (hasTransform) {
        ...
        if (auto pathOperation = renderer().style().offsetPath(); pathOperation && is<BoxPathOperation>(pathOperation)) {
            ...
            auto pathReferenceBoxRect = snapRectToDevicePixelsIfNeeded(renderer().referenceBoxRect(boxPathOperation.referenceBox()));
            boxPathOperation.setPathForReferenceRect(FloatRoundedRect { pathReferenceBoxRect });
        }

        auto referenceBoxRect = snapRectToDevicePixelsIfNeeded(renderer().transformReferenceBoxRect(renderer().style()));
...
TransformationMatrix RenderLayer::currentTransform(OptionSet<RenderStyle::TransformOperationOption> options) const
{
    ...
    if ((styleable && styleable->isRunningAcceleratedTransformAnimation()) || !options.contains(RenderStyle::TransformOperationOption::TransformOrigin)) {
        ...
        auto referenceBoxRect = snapRectToDevicePixelsIfNeeded(renderer().transformReferenceBoxRect(*animatedStyle));
...
TransformationMatrix RenderLayer::perspectiveTransform(const LayoutRect& layerRect) const
{
    ...
    auto referenceBoxRect = snapRectToDevicePixelsIfNeeded(renderer().transformReferenceBoxRect(style));
    auto snappedLayerRect = snapRectToDevicePixelsIfNeeded(layerRect);
...
bool RenderLayer::rendererNeedsPixelSnapping() const
{
#if ENABLE(LAYER_BASED_SVG_ENGINE)
...
FloatRect RenderLayer::snapRectToDevicePixelsIfNeeded(const LayoutRect& rect) const
{
    ASSERT(rendererNeedsPixelSnapping());
    return snapRectToDevicePixels(rect, renderer().document().deviceScaleFactor());
}

FloatRect RenderLayer::snapRectToDevicePixelsIfNeeded(const FloatRect& rect) const
{
    if (!rendererNeedsPixelSnapping())
        return rect;
    return snapRectToDevicePixelsIfNeeded(LayoutRect { rect });
}
...
std::pair<Path, WindRule> RenderLayer::computeClipPath(const LayoutSize& offsetFromRoot, const LayoutRect& rootRelativeBoundsForNonBoxes) const
{
    ...
    if (is<ShapePathOperation>(*style.clipPath())) {
        ...
        auto referenceBoxRect = referenceBoxRectForClipping(clipPath.referenceBox(), offsetFromRoot, rootRelativeBoundsForNonBoxes);
        auto snappedReferenceBoxRect = snapRectToDevicePixelsIfNeeded(referenceBoxRect);
...
void RenderLayer::setupClipPath(GraphicsContext& context, GraphicsContextStateSaver& stateSaver, const LayerPaintingInfo& paintingInfo, const LayoutSize& offsetFromRoot)
{
    ...
    if (is<ReferencePathOperation>(style.clipPath())) {
        ...
        if (auto* clipperRenderer = renderer().ensureReferencedSVGResources().referencedClipperRenderer(renderer().document(), referenceClipPathOperation)) {
            ...
            // clippedContentBounds is used as the reference box for inlines, which is also poorly specified: https://github.com/w3c/csswg-drafts/issues/6383.
            auto referenceBox = referenceBoxRectForClipping(CSSBoxType::BorderBox, offsetFromRoot, clippedContentBounds);
            auto snappedReferenceBox = snapRectToDevicePixelsIfNeeded(referenceBox);
            auto offset = snappedReferenceBox.location();

            auto snappedClippingBounds = snapRectToDevicePixelsIfNeeded(clippedContentBounds);
