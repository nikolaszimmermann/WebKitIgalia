...
static inline FloatOrLayoutRect computeReferenceBox(const RenderLayerModelObject& renderer, CSSBoxType boxType, const LayoutSize& offsetFromRoot, LayoutRect rootRelativeBounds)
{
...
void RenderLayer::updateTransform()
{
    ...
    if (hasTransform) {
        ...
        auto referenceBoxRectForPainting = renderer().transformReferenceBoxRect().floatRectForPainting(renderer().document().deviceScaleFactor());
...
TransformationMatrix RenderLayer::currentTransform(OptionSet<RenderStyle::TransformOperationOption> options) const
{
    ...
    if ((styleable && styleable->isRunningAcceleratedTransformAnimation()) || !options.contains(RenderStyle::TransformOperationOption::TransformOrigin)) {
        ...
        auto referenceBoxRectForPainting = renderBox.transformReferenceBoxRect().floatRectForPainting(renderBox.document().deviceScaleFactor());
...
TransformationMatrix RenderLayer::perspectiveTransform(const LayoutRect& layerRect) const
{
    ...
    auto referenceBoxRectForPainting = renderBox.transformReferenceBoxRect().floatRectForPainting(deviceScaleFactor);
...
std::pair<Path, WindRule> RenderLayer::computeClipPath(const LayoutSize& offsetFromRoot, const LayoutRect& rootRelativeBoundsForNonBoxes) const
{
    ...
    if (is<ShapePathOperation>(*style.clipPath())) {
        ...
        auto referenceBoxRect = computeReferenceBox(renderer(), clipPath.referenceBox(), offsetFromRoot, rootRelativeBoundsForNonBoxes);
        return { clipPath.pathForReferenceRect(referenceBoxRect.floatRectForPainting(deviceScaleFactor)), clipPath.windRule() };
    }
...
void RenderLayer::setupClipPath(GraphicsContext& context, GraphicsContextStateSaver& stateSaver, const LayerPaintingInfo& paintingInfo, const LayoutSize& offsetFromRoot)
{
    ...
    if (is<ReferencePathOperation>(style.clipPath())) {
        ...
        if (auto* clipperRenderer = renderer().ensureReferencedSVGResources().referencedClipperRenderer(renderer().document(), referenceClipPathOperation)) {
            ...
            FloatRect referenceBoxRect = referenceBox;
            referenceBoxRect.setLocation({ });

            auto referenceBoxRectForPainting = referenceBox.floatRectForPainting(renderer().document().deviceScaleFactor());
            auto offset = referenceBoxRectForPainting.location();
            ...
            clipperRenderer->applyClippingToContext(context, renderer(), referenceBoxRect, snappedClippingBounds, renderer().style().effectiveZoom());
            context.translate(-offset);
        }
    }
}
