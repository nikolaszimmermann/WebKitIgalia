...
FloatRect RenderLayer::referenceBoxRect(CSSBoxType boxType, const LayoutSize& offsetFromRoot, const LayoutRect& rootRelativeBounds) const
{
    if (auto referenceBoxRect = renderer().referenceBoxFloatRect(boxType))
    ...
    auto referenceBoxRect = renderer().referenceBoxLayoutRect(boxType).value();
    referenceBoxRect.move(offsetFromRoot);
    return referenceBoxRect;
}

FloatRect RenderLayer::referenceBoxRectForPainting(CSSBoxType boxType, const LayoutSize& offsetFromRoot, const LayoutRect& rootRelativeBounds) const
{
    if (auto referenceBoxRect = renderer().referenceBoxFloatRect(boxType))
        return referenceBoxRect.value();
...
std::pair<Path, WindRule> RenderLayer::computeClipPath(const LayoutSize& offsetFromRoot, const LayoutRect& rootRelativeBoundsForNonBoxes) const
{
    ...
    if (is<ShapePathOperation>(*style.clipPath())) {
        ...
        auto referenceBoxRect = referenceBoxRectForPainting(clipPath.referenceBox(), offsetFromRoot, rootRelativeBoundsForNonBoxes);
        return { clipPath.pathForReferenceRect(referenceBoxRect), clipPath.windRule() };
    }
...
void RenderLayer::setupClipPath(GraphicsContext& context, GraphicsContextStateSaver& stateSaver, const LayerPaintingInfo& paintingInfo, const LayoutSize& offsetFromRoot)
{
    ...
    if (is<ReferencePathOperation>(style.clipPath())) {
        ...
        if (auto* clipperRenderer = renderer().ensureReferencedSVGResources().referencedClipperRenderer(renderer().document(), referenceClipPathOperation)) {
            ...
            // clippedContentBounds is used as the reference box for inlines, which is also poorly specified: https://github.com/w3c/csswg-drafts/issues/6383.
            auto clipPathReferenceBox = referenceBoxRect(CSSBoxType::BorderBox, offsetFromRoot, clippedContentBounds);
            auto clipPathReferenceBoxForPainting = referenceBoxRectForPainting(CSSBoxType::BorderBox, offsetFromRoot, clippedContentBounds);
            auto offset = clipPathReferenceBoxForPainting.location();
            ...
            clipperRenderer->applyClippingToContext(context, renderer(), { { }, clipPathReferenceBox.size() }, snappedClippingBounds, renderer().style().effectiveZoom());
            context.translate(-offset);
        }
    }
}
