...
std::optional<LayoutRect> RenderLayerModelObject::computeVisibleRectInContainer(const LayoutRect& rect, const RenderLayerModelObject* container, RenderObject::VisibleRectContext context) const
{
    // FIXME: [LBSE] Upstream RenderSVGBlock changes
    // if (!is<RenderSVGModelObject>(*this) && !is<RenderSVGBlock>(this)) {
    if (!is<RenderSVGModelObject>(*this))
        return RenderElement::computeVisibleRectInContainer(rect, container, context);
...
void RenderLayerModelObject::mapLocalToContainer(const RenderLayerModelObject* ancestorContainer, TransformState& transformState, OptionSet<MapCoordinatesMode> mode, bool* wasFixed) const
{
    if (!is<RenderSVGModelObject>(*this))
        return RenderElement::mapLocalToContainer(ancestorContainer, transformState, mode, wasFixed);
