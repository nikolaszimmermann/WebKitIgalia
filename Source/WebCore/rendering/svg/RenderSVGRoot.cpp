...
void RenderSVGRoot::layout()
{
    ...
    SVGLogger::DebugScope debugScope(
        ...
        [&]() {
            showSVGRenderTreeForThis();
            // FIXME SVGContainerLayout::verifyLayoutLocationConsistency(*this);
        });
#endif
    ...
    LayoutRepainter repainter(*this, checkForRepaintDuringLayout());
    {
        ...
        updateLogicalWidth();
        updateLogicalHeight();
        m_isLayoutSizeChanged = selfNeedsLayout() || (svgSVGElement().hasRelativeLengths() && previousLogicalSize != size());
...
void RenderSVGRoot::mapLocalToContainer(const RenderLayerModelObject* repaintContainer, TransformState& transformState, OptionSet<MapCoordinatesMode> mode, bool* wasFixed) const
{
    ...
    if (computingCTMOrScreenCTM) {
        ...
        if (transformState.transformMatrixTracking() == TransformState::TrackSVGScreenCTMMatrix) {
            ...
            auto transformAboveSVGFragment = transformStateAboveSVGFragment.releaseTrackedTransform();
