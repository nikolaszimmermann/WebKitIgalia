...
SVGContainerLayout::~SVGContainerLayout()
{
}
...
#if !defined(NDEBUG)
        SVGLogger::debug([&](TextStream& stream) {
            stream << "--> SVGContainerLayout m_container " << &m_container << " (" << m_container.renderName() << ")"
            << " - moving child renderer " << &child << " (" << child.renderName() << ")"
            << " from layoutLocation=" << currentLayoutLocation <<
            " to layoutLocation=" << desiredLayoutLocation;
        });
#endif
    }
}
...
void SVGContainerLayout::verifyLayoutLocationConsistency(const RenderElement& renderer)
{
    ...
    if (renderer.isSVGRoot()) {
        SVGLogger::debug([&](TextStream& stream) {
            stream << "--> SVGContainerLayout renderer " << &renderer << " (" << renderer.renderName() << ")" << " - verifyLayoutLocationConsistency() begin";
        });
    }
#endif
    ...
    if (renderer.isSVGLayerAwareRenderer() && !renderer.isSVGRoot()) {
        ...
        if (expectedLayoutLocation == initialLayoutLocation) {
            SVGLogger::info([&](TextStream& stream) {
                stream << "--> SVGContainerLayout renderer " << &renderer << " (" << renderer.renderName() << ")"
            ...
            SVGLogger::error([&](TextStream& stream) {
                ...
                << " -> aborting with a render tree / SVG tree dump";
            });

#if ENABLE(TREE_DEBUGGING)
            showRenderTree(&renderer);
            showSVGRenderTree(&renderer);
#endif
